...
function BoardForm(props) {
    ...
    useEffect(() => {
        getApiRequest(BOARDS_URL + '/types').then(types => setAttachmentTypes(
            types.map(type => {
                return {
                    'key': type.name,
    ...
    return (
        <Form onSubmit={handleSubmit} className={'six wide column centered'} error={errors !== undefined}>
            <Segment>
                <Header as='h4' dividing>Create new board</Header>
                <Form.Input label='Label' placeholder='Label' value={label}
                            onChange={e => setLabel(e.target.value)} required/>
                <Form.Input label='Name' placeholder='Name' value={name}
                            onChange={e => setName(e.target.value)} required/>
                <Form.Select label='Allowed attachment types' options={attachmentTypes}
                             onChange={(e, {value}) => setAttachmentType(value)} required/>
                <Form.Checkbox label='NSFW' checked={nsfw} onChange={() => setNsfw(!nsfw)}/>
